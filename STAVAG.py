...
def DVG_detection(
    ...
    Returns:
        ...
        - For num_perm > 1: each DataFrame contains columns such as:
              ['Feature', 'Importance', 'null_mean', 'pval']
          and is filtered by p-value (<= threshold).
    """
    np.random.seed(0)
...
def TVG_detection(
    ...
    Returns:
        ...
        - For num_perm > 1: the DataFrame under key 'T' contains columns:
              ['Feature', 'Importance', 'null_mean', 'pval']
          and is filtered by p-value (<= threshold).
    """
    np.random.seed(0)
